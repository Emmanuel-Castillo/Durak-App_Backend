...
// GOOD
export function startGame(room, durak) {
    try {
        const newGame = {
            turn: 1,
            deck: [],
        ...
        room.users.forEach((u, index) => {
            newGame.players.push({
                ...
                role: "FirstAttacker",
                nextPlayerUserId: room.users[(index + 1) % room.users.length].account_id,
            });
        ...
        return {
            ...
            game: room.game,
            error: null,
            message: `Game started! ${firstAttacker.user.username} has the first move...`,
        };
    }
    catch (error) {
...
// GOOD
export function firstMoveDealt(userId, atkCard, game) {
    try {
        // Validate if game is in FirstMove state
        if (game.gameState !== "FirstMove")
            throw new Error("firstMoveDealt(): Attempting to deal first move outside of FirstMove game state");
...
// GOOD
export function attackMove(userId, atkCard, game) {
    try {
        // Validate if user is attacker
        const attackerPlayer = fetchPlayer(game, userId);
        if (!attackerPlayer)
            throw new Error("attackMove(): Player not found!");
        ...
        if (game.playedCards.length >= 6) {
            throw new Error("attackMove(): playedCards has reached limit of 6 elements");
        }
        // Validate if atkCard follows game rules!
        let atkCardMatchesValue = false;
        game.playedCards.forEach((p) => {
            if (p.attackingCard.value === atkCard.value ||
        ...
        return {
            ...
            game,
            error: null,
            message: "Attack: " + formatPlayerDealsCardMessage(attackerPlayer, atkCard),
        };
    }
    catch (error) {
        return { success: false, game: null, error: error, message: null };
...
// GOOD
export function endAttackerTurn(userId, game) {
    try {
        // Validate if user is attacker
        const attackerPlayer = fetchPlayer(game, userId);
        if (!attackerPlayer)
...
// GOOD
export function defendMove(userId, defCard, cardPair, game) {
    try {
        // Validate if user is defender
        const defenderPlayer = fetchPlayer(game, userId);
        if (!defenderPlayer)
            throw new Error("defendMove(): Player not found!");
        ...
        if (!doesDefCardDefend(defCard, playedCard, game.tsarCard))
            throw new Error("defendMove(): Card can't defend against playedCard pair!");
        // Update game
        ...
        return {
            ...
            game,
            error: null,
            message: "Defend: " + formatPlayerDealsCardMessage(defenderPlayer, defCard),
        };
    }
    catch (error) {
        return { success: false, game: null, error: error, message: null };
...
// GOOD
export function counterMove(userId, counterCard, game) {
    try {
        // Validate if user is defender
        const defenderPlayer = fetchPlayer(game, userId);
        if (!defenderPlayer)
            throw new Error("counterMove(): Player not found!");
        if (defenderPlayer.role !== "Defender")
        ...
        game.playedCards.forEach((p) => {
            if (p.defendingCard)
                throw new Error("counterMove(): Player has already defended! Counter is invalid.");
        });
        // Update game
        ...
        if (!defPlayerNextPlayer)
            throw new Error("Cannot fetch next player!");
        ...
        return {
            ...
            game,
            error: null,
            message: "Counter: " + formatPlayerDealsCardMessage(defenderPlayer, counterCard),
        };
    }
    catch (error) {
        return { success: false, game: null, error: error, message: null };
...
// GOOD
export function yieldTurn(userId, game) {
    try {
        // Validate if user is defender
        const defenderPlayer = fetchPlayer(game, userId);
        if (!defenderPlayer)
            throw new Error("yieldTurn(): Player not found!");
        if (defenderPlayer.role !== "Defender")
        ...
        if (!firstAttacker)
            throw new Error("yieldTurn(): Players' roles were not reassigned");
        game.turn += 1;
        game.gameState = "FirstMove";
        return {
            success: true,
...
function hasPlayerFinishedGame(player, game) {
    return player.hand.length === 0 && game.deck.length === 0;
}
function moveToWinnersList(finishedPlayer, game) {
...
function doesDefCardDefend(defCard, playedCards, tsarCard) {
    // Base cases :
...
function dealCardsToPlayers(game) {
    ...
    const startTurnFirstAttacker = players.find((p) => p.startTurnRole === "FirstAttacker");
    if (!startTurnFirstAttacker)
        return;
    let deckSliceOffset = Math.max(6 - startTurnFirstAttacker.hand.length, 0);
    startTurnFirstAttacker.hand = startTurnFirstAttacker.hand.concat(game.deck.slice(0, deckSliceOffset));
    ...
    defenderPlayer.hand = defenderPlayer.hand.concat(game.deck.slice(0, deckSliceOffset));
    game.deck = game.deck.slice(deckSliceOffset);
}
function formatPlayerDealsCardMessage(player, card) {
    return `${player.user.username} deals the ${card.value} of ${card.suit}`;
}
function assignPlayerRoles(game, firstAttackerUserId) {
    let firstAttackerIndex = game.players.findIndex((p) => p.user.account_id === firstAttackerUserId);
